...
- **[production]** [code-assembly-skill](https://github.com/baoyu0/skills/tree/main/methodology/code-assembly) by [baoyu0](https://github.com/baoyu0) - Search-first methodology skill for AI coding agents. 7-step workflow: define requirements → reverse-search mature toolchains → evaluate candidates → select with decision records → design adapter boundaries → generate glue code only → verify delivery. Fights the "implementer instinct" with an "integrator mindset." Real-world case studies from Hermes production use. Includes 4 reference templates and works with Hermes, Claude Code, and Codex. Install: `npx skills add baoyu0/skills@methodology/code-assembly -g -y`. MIT.
